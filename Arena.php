...
class Arena implements Listener {
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $this->broadcastMessage("§f{$player->getName()}§e leave the game §e(§b".count($this->players)."/{$this->data["slots"]}§e)");
        }
    ...
    public function startRestart() {
		...
		$player->sendMessage("\n");
		$player->sendMessage("                   §l§aYou Win!                    ");
		$player->sendMessage("§eEarned §690§9 Coins§e for Winning");
    ...
    public function onDeath(PlayerDeathEvent $event) {
        ...
        $this->broadcastMessage("§7{$this->plugin->getServer()->getLanguage()->translate($event->getDeathMessage())}§e §b(".count($this->players)."/{$this->data["slots"]})");
